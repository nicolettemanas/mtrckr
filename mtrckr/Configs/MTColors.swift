//
//  MTColors.swift
//  mtrckr
//
//  Created by User on 6/8/17.
//
//

import UIKit
import UIColor_Hex_Swift

struct MTColors {
    static let mainBg = UIColor("#FFFFFFFF")
    static let barBg = UIColor("#FFFFFFFF")
    static let separatorColor = UIColor("#DDDDDDFF")
    
    static let mainText = UIColor("#34374BFF")
    static let subText = UIColor("#6C6F87FF")
    static let placeholderText = UIColor("#A3A5B7FF")
    static let lightBg = UIColor("#EEEEEEFF")
    
    static let mainBlue = UIColor("#64ABDAFF")
    static let mainRed = UIColor("#F49899FF")
    static let mainOrange = UIColor("#F5AC72FF")
    
    static let colors: [UIColor] = [UIColor("#F49A50FF"), // orange
                                    UIColor("#F47072FF"), // red orange
                                    UIColor("#479ED8FF"), // dark blue
                                    UIColor("#CE5F7EFF"), // pale pink
                                    UIColor("#CE5A9AFF"), // hot pink
                                    UIColor("#38A55EFF"), // green
                                    UIColor("#59C0E5FF"), // light blue
                                    UIColor("#B558B3FF"), // violet
                                    UIColor("#898D9EFF"), // gray
                                    UIColor("#686BC9FF")  // purple
                                    ]
}
