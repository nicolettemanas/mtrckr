...
class TransactionTests: QuickSpec {
        ...
        describe("CRUD operations") {
                    ...
                    describe("update: change of type", { 
                        ...
                        context("update is a change of type income to expense", {
                            it("should update account's current amount, add to total expenses and subtract to total income", closure: {
                                transactionSaved = Transaction(type: .income, name: "Salary", image: nil, description: "January Salary", amount: 10000,
                                                               category: newCategory, from: account, to: account, date: transDate)
                                transactionSaved.save(toRealm: self.testRealm)
                                
                                transactionSaved.update(type: .expense, name: "Breakfast", image: nil, description: "Subway: Spicy Italian", amount: 99.0, category: category, from: account, to: account, date: transDate, inRealm: self.testRealm)
                        ...
                        context("update is a change of type from income/expense to transfer", {
                            it("should update account's current ammount, add expense and reset income", closure: {
                                ...
                                transactionSaved.update(type: .transfer, name: "Allowance", image: nil, description: "Cash allowance", amount: 500,
                                                        category: nil, from: account, to: newAccount, date: transDate, inRealm: self.testRealm)
                        ...
                        context("update is a change of type from transfer to income/expense", {
                            it("should update account's current ammount, reset expense and add to income", closure: {
                                transactionSaved = Transaction(type: .transfer, name: "Allowance", image: nil, description: "Cash allowance", amount: 500,
                                                               category: nil, from: account, to: newAccount, date: transDate)
                                transactionSaved.save(toRealm: self.testRealm)
            ...
            describe("delete()", {
                ...
                beforeEach {
                    ...
                    expenseTransaction = Transaction(type: .expense, name: "Transaction", image: nil, description: "Desc",
                                                     amount: 100, category: category, from: account, to: account, date: Date())
                    incomeTransaction = Transaction(type: .income, name: "Salary", image: nil, description: "January Salary",
                                                    amount: 10000, category: incomeCategory, from: account, to: account, date: Date())
                    transferTransaction = Transaction(type: .transfer, name: "Allowance", image: nil, description: "Cash allowance",
                                                      amount: 500, category: nil, from: account, to: newAccount, date: transDate)
                ...
                context("deleted transaction is an expense", { 
                    ...
                    context("deleted transaction has associated bill", {
                        it("bill entry should be marked as unpaid") {
                            ...
                            entry.pay(amount: 3000, description: "", fromAccount: account, datePaid: Date(), inRealm: self.testRealm)
    ...
    func createTransactions(n: Int) {
        ...
        for i in 0..<n {
            let transaction = Transaction(type: .expense, name: "Transaction \(i)", image: nil, description: "Desc \(i)",
                                      amount: 100, category: category, from: account, to: account, date: Date())
            transaction.save(toRealm: self.testRealm)
        }
    }
